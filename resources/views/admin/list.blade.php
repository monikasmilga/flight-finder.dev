...
@if($pageTitle == 'Countries')
        <nav aria-label="Page navigation example">
            <ul class="pagination">
                ...
                @else
                    <li class="page-item"><a class="page-link" href="{{route('app.countries.index')}}?page={{$list["current_page"]-1}}">Previous</a></li>
                @endif

                @for($i=1; $i<=$list['last_page']; $i++)
                    <li class="page-item"><a class="page-link" href="{{route('app.countries.index')}}?page={{$i}}">{{$i}}</a></li>
                @endfor

                @if(($list["current_page"]) === $list['last_page'])
                    <li class="disabled-link"><a id="disabled-link">Next</a></li>
                @else
                    <li class="page-item"><a class="page-link" href="{{route('app.countries.index')}}?page={{$list["current_page"]+1}}">Next</a></li>
                @endif
            </ul>
        </nav>
